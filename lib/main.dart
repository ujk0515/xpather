...
class _BrowserPageState extends State<BrowserPage> {
  ...
  List<ElementInfo> _customExtractedElements = []; // 커스텀 모드에서 추출된 요소들
  ...
  void _startExtractionTimer() {
    _extractionTimer = Timer.periodic(const Duration(milliseconds: 100), (timer) async {
      ...
      try {
        final result = await _controller.executeScript('window.xpatherGetExtracted()');

        if (result != null && result is Map) {
          final elementData = Map<String, dynamic>.from(result);

          // className이 객체일 수 있으므로 문자열로 변환
          String className = '';
          if (elementData['className'] != null) {
            if (elementData['className'] is String) {
              className = elementData['className'];
            } else {
              className = elementData['className'].toString();
            }
          }

          final elementInfo = ElementInfo(
            tag: elementData['tag']?.toString() ?? '',
            text: elementData['text']?.toString() ?? '',
            xpath: elementData['xpath']?.toString() ?? '',
            id: elementData['id']?.toString() ?? '',
            className: className,
            name: elementData['name']?.toString() ?? '',
            type: elementData['type']?.toString() ?? '',
            placeholder: elementData['placeholder']?.toString() ?? '',
          );
...
class _ResultsPanelState extends State<_ResultsPanel> with SingleTickerProviderStateMixin {
  ...
  Future<void> _highlightElement(String xpath) async {
    ...
    try {
      final result = await widget.controller.executeScript(script);
      if (result == 'NOT_FOUND') {
  ...
  Future<void> _clearHighlight() async {
    ...
    try {
      await widget.controller.executeScript(script);
    } catch (e) {
